let json = `[
    {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  },
  {
    "date": "1986-08-31T23:55:22Z",
    "authorName": "Успенский",
    "authorUrl": "https://www.happy-kids.ru/page.php?id=1105",
    "content": "Я был когда-то странной\nИгрушкой безымянной,\nК которой в магазине\nНикто не подойдет.\nТеперь я Чебурашка,\nМне каждая дворняжка\nПри встрече сразу\nЛапу подает!\n\nМне не везло сначала,\nИ даже так бывало:\nКо мне на день рожденья\nНикто не приходил.\nТеперь я вместе с Геной\nОн не обыкновенный,\nОн самый лучший\nВ мире крокодил! ",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 24,
        "tone": -0.6,
        "length": 8
      },
      {
        "position": 66,
        "tone": 0.3,
        "length": 5
      },
      {
        "position": 94,
        "tone": 0.9,
        "length": 9
      },
      {
        "position": 109,
        "tone": 1,
        "length": 16
      },
      {
        "position": 144,
        "tone": 1,
        "length": 4
      },
      {
        "position": 158,
        "tone": 0,
        "length": 3
      },
      {
        "position": 199,
        "tone": 0.3,
        "length": 6
      },
      {
        "position": 260,
        "tone": 0.7,
        "length": 5
      },
      {
        "position": 309,
        "tone": 0.3,
        "length": 8
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "http://hyaenidae.narod.ru/story1/032.html",
    "content": "Я Колобок, Колобок,\n Я по коробу скребен,\n По сусеку метен,\n На сметане мешон\n Да в масле пряжон,\n На окошке стужон.\n Я от дедушки ушел,\n Я от бабушки ушел,\n От тебя, зайца, подавно уйду!",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.5,
        "length": 3
      },
      {
        "position": 87,
        "tone": -0.9,
        "length": 7
      },
      {
        "position": 95,
        "tone": 1,
        "length": 7
      },
      {
        "position": 152,
        "tone": 0.3,
        "length": 10
      },
      {
        "position": 164,
        "tone": 0.1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "В  одном сказочном городе жили коротышки. Коротышками их называли потому,\n       что они были очень  маленькие.  Каждый  коротышка  был  ростом  с  небольшой\n       огурец.  В городе у них было очень красиво. Вокруг каждого дома росли цветы:\n     маргаритки, ромашки, одуванчики. Там даже улицы назывались  именами  цветов:\n       улица Колокольчиков, аллея Ромашек, бульвар Васильков. А сам город назывался\n     Цветочным  городом.  Он стоял на берегу ручья. Этот ручей коротышки называли\n     Огурцовой рекой, потому что по берегам ручья росло много огурцов.\n       За рекой был лес. Коротышки делали из березовой коры лодочки, переплывали\n     через реку и ходили в лес за ягодами, за грибами, за орехами. Собирать ягоды\n     было трудно, потому что коротышки ведь были крошечные, а за орехами и  вовсе\n     приходилось  лазить  на  высокий  куст  да  еще тащить с собой пилу. Ни один\n     коротышка не смог бы сорвать орех руками -- их надо было пилить пилой. Грибы\n     тоже пилили пилой. Спилят гриб под самый корень, потом распилят его на части\n     и тащат по кусочкам домой.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 1
      },
      {
        "position": 1,
        "tone": -0.9,
        "length": 1
      },
      {
        "position": 2,
        "tone": -0.8,
        "length": 1
      },
      {
        "position": 3,
        "tone": -0.7,
        "length": 1
      },
      {
        "position": 4,
        "tone": -0.6,
        "length": 1
      },
      {
        "position": 5,
        "tone": -0.5,
        "length": 1
      },
      {
        "position": 6,
        "tone": -0.4,
        "length": 1
      },
      {
        "position": 7,
        "tone": -0.3,
        "length": 1
      },
      {
        "position": 8,
        "tone": -0.2,
        "length": 1
      },
      {
        "position": 9,
        "tone": -0.1,
        "length": 1
      },
      {
        "position": 10,
        "tone": 0,
        "length": 1
      },
      {
        "position": 11,
        "tone": 0.1,
        "length": 1
      },
      {
        "position": 12,
        "tone": 0.2,
        "length": 1
      },
      {
        "position": 13,
        "tone": 0.3,
        "length": 1
      },
      {
        "position": 14,
        "tone": 0.4,
        "length": 1
      },
      {
        "position": 15,
        "tone": 0.5,
        "length": 1
      },
      {
        "position": 16,
        "tone": 0.6,
        "length": 1
      },
      {
        "position": 17,
        "tone": 0.7,
        "length": 1
      },
      {
        "position": 18,
        "tone": 0.8,
        "length": 1
      },
      {
        "position": 19,
        "tone": 0.9,
        "length": 1
      },
      {
        "position": 20,
        "tone": 1,
        "length": 1
      }
    ]
  },
  {
    "date": "1980-08-31T23:55:22Z",
    "authorName": "Чуковский",
    "authorUrl": "https://www.stihi-rus.ru/1/chukovskiy/14.htm",
    "content": "Тут и мыло подскочило\n И вцепилось в волоса,\n И юлило, и мылило,\n И кусало, как оса.\n А от бешеной мочалки\n Я помчался, как от палки,\n А она за мной, за мной\n По Садовой, по Сенной.",
    "contentPostTones": [
      {
        "position": 77,
        "tone": -0.4,
        "length": 3
      },
      {
        "position": 87,
        "tone": 0.8,
        "length": 7
      },
      {
        "position": 95,
        "tone": -0.8,
        "length": 7
      },
      {
        "position": 152,
        "tone": -0.6,
        "length": 10
      },
      {
        "position": 164,
        "tone": 1,
        "length": 9
      }
    ]
  },
  {
    "date": "1999-08-31T23:55:22Z",
    "authorName": "Николай Носов",
    "authorUrl": "http://lib.ru/NOSOW/nezn1.txt",
    "content": "Ниф-Ниф решил, что проще и скорее всего смастерить дом из соломы. Ни с кем не посоветовавшись, он так и сделал. Уже к вечеру его хижина была готова. Ниф-Ниф положил на крышу последнюю соломинку и, очень довольный своим домиком, весело запел:",
    "contentPostTones": [
      {
        "position": 0,
        "tone": -1,
        "length": 7
      },
      {
        "position": 58,
        "tone": -1,
        "length": 6
      },
      {
        "position": 149,
        "tone": 1,
        "length": 7
      },
      {
        "position": 228,
        "tone": 1,
        "length": 12
      }
    ]
  },
  {
    "date": "1697-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/112-krasnaya-shapochka.html",
    "content": "К счастью, мимо проходил лесник. Он уже издали заметил, что случилось что-то неладное: двери домика были распахнуты настежь, и оттуда доносился громкий храп. Лесник снял с плеча двустволку и подкрался к окну. Он чуть не вскрикнул, увидев развалившегося на бабушкиной кровати волка с вздувшимся брюхом. Не раздумывая, лесник вбежал в дом, выхватил из-за пояса охотничий нож и мгновенно распорол волку брюхо. Оттуда выскочила Красная Шапочка, а за ней и бабушка. Ох, как темно было в брюхе у волка! Страшно даже подумать, что бы было, не приди храбрый и находчивый лесник вовремя.",
    "contentPostTones": [
      {
        "position": 0,
        "tone": 0,
        "length": 578
      }
    ]
  },
  {
    "date": "1856-08-31T23:55:22Z",
    "authorName": "Шарль Перро",
    "authorUrl": "https://dlya-detey.com/skazki/sharlya-perro/113-malchik-s-palchik.html",
    "content": "Так и стал Мальчик-с-пальчик королевским гонцом и несколько лет носился по всему свету с королевскими письмами и поручениями. Когда же он скопил достаточное состояние, да и семимильные сапоги до дыр стерлись, вернулся в родной дом на опушке леса.",
    "contentPostTones": [
      {
        "position": 11,
        "tone": -0.7,
        "length": 17
      },
      {
        "position": 50,
        "tone": 0.5,
        "length": 9
      },
      {
        "position": 89,
        "tone": -0.3,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/142-baba-yaga.html",
    "content": "Вот тебе гребешок и полотенце, - говорит кот, - возьми их и убежи; за тобою будет гнаться баба-яга, ты приклони ухо к земле и, как заслышишь, что она близко, брось сперва полотенце - сделается широкая-широкая река; если ж баба-яга перейдет через реку и станет догонять тебя, ты опять приклони ухо к земле и, как услышишь, что она близко, брось гребешок - сделается дремучий-дремучий лес, сквозь него она уже не проберется!",
    "contentPostTones": [
      {
        "position": 9,
        "tone": -1,
        "length": 20
      },
      {
        "position": 89,
        "tone": -0.7,
        "length": 36
      },
      {
        "position": 231,
        "tone": 0.9,
        "length": 14
      }
    ]
  },
  {
    "date": "1970-08-31T23:55:22Z",
    "authorName": "Неизвестен",
    "authorUrl": "https://dlya-detey.com/skazki/russkie-narodnie/24-lisa-i-zhuravl-chitat.html",
    "content": "Журавль хлоп-хлоп носом, стучал-стучал, ничего не попадает. А лисица в это время лижет себе да лижет кашу — так всю сама и слизала. Каша съедена; лисица и говорит:\n       — Не обессудь, любезный кум! Больше угощать нечем!\n — Спасибо, кума, и на этом! Приходи теперь ко мне в гости.",
    "contentPostTones": [
      {
        "position": 25,
        "tone": 0.8,
        "length": 13
      },
      {
        "position": 60,
        "tone": -0.6,
        "length": 26
      },
      {
        "position": 199,
        "tone": -0.3,
        "length": 21
      }
    ]
  },
  {
    "date": "1826-08-31T23:55:22Z",
    "authorName": "Пушкин",
    "authorUrl": "https://dlya-detey.com/skazki/skazki-pushkina/32-u-lukomorya-dub-zelenyy-chitat.html",
    "content": "И днём и ночью кот учёный\n Всё ходит по цепи кругом;\n Идёт направо - песнь заводит,\n Налево - сказку говорит.\n Там чудеса: там леший бродит,\n Русалка на ветвях сидит;\n Там на неведомых дорожках\n Следы невиданных зверей;\n Избушка там на курьих ножках\n Стоит без окон, без дверей;\n Там лес и дол видений полны;",
    "contentPostTones": [
      {
        "position":15,
        "tone": 1,
        "length": 10
      },
      {
        "position": 53,
        "tone": -0.1,
        "length": 29
      },
      {
        "position": 185,
        "tone": -1,
        "length": 33
      }
    ]
  }]`;

//Исключить спец. символы
function jsonEscape(jsonString) {
  return jsonString.replace(/\n/g, " ");
}

let arr = JSON.parse(jsonEscape(json));

new Vue({
  el: '#app',
  data:{
    lines:[
      {
        date: new Date(arr[0].date),
        author: arr[0].authorName,
        url: arr[0].authorUrl,
        content: arr[0].content,
      }
    ],
  },
  methods:{
    addType(){
      //Обернуть искомые элементы в <span>
      for(let a = 1; a < arr.length; a++){
        this.lines.push({date : new Date(arr[a].date) + ' / ', author : arr[a].authorName + ' / ', url : arr[a].authorUrl, content: arr[a].content})
      }
    }
  },
  beforeMount(){
    this.addType()
  },
})


let cont = document.getElementById('app').querySelectorAll("p");
/* for(let a = 0; a < arr.length; a++){ */
  let a = 1;
  let letLength = 0;
  for (let k = 0; k < arr[a].contentPostTones.length; k++){
      let tone = arr[a].contentPostTones[k].tone;
      let start = arr[a].contentPostTones[k].position + 22 * k + letLength;
      let end = arr[a].contentPostTones[k].position + arr[a].contentPostTones[k].length + 22 * k + letLength;

      let color = []; 

      let content = cont[a].innerHTML;

      for (let i = start; i < end; i++){
          color = color +  content[i]; 
      }
      cont[a].innerHTML = cont[a].innerHTML.replace(color, `<span class="${tone}">$&</span>`);

      //длина класса
      letLength = letLength + tone.toString().length;

      
      
      console.log(content);
      console.log('');
      console.log(start);
      console.log(end);
      console.log(tone);
      console.log(color);
      console.log(letLength);
      console.log('');
  }
/* } */

/* "contentPostTones": [
  {
    "position": 77,
    "tone": -0.5,
    "length": 3
  },
  {
    "position": 87,
    "tone": -0.9,
    "length": 7
  },
  {
    "position": 95,
    "tone": 1,
    "length": 7
  },
  {
    "position": 152,
    "tone": 0.3,
    "length": 10
  },
  {
    "position": 164,
    "tone": 0.1,
    "length": 9
  }
] */